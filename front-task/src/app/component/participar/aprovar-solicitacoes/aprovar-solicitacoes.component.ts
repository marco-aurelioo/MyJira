...
export class AprovarSolicitacoesComponent implements OnInit {
  ...
  pageProjects: PageProjetoInvites = { content : []};
  ...
  constructor(
    ...
    buscaMinhasAprovacoes(pagina: number){
      ...
      this.participarService.findMyProjetosParaAprovar(pagina,5,"").subscribe(
        (page) => {
          console.log(page);
    ...
    carregaInvites(projeto: Projeto){
      ...
      this.participarService.findParticipacoes(projeto,this.currentPageSolicitacoes,5).subscribe(
        (page) => {
    ...
    responderSolicitacao(){
      ...
      this.mostrarModalResposta = false;
      this.participarService.atualizaPArticipacoes(this.solicitacaoSelecionada!,this.acaoResposta!).subscribe(
        (page) => {
